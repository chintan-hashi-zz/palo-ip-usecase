...
data "azurerm_public_ip" "exists" {
  ...
  resource_group_name = try(each.value.public_ip_resource_group_name, var.resource_group.name)
}
...
locals {
  ...
  frontend_ips = { for k, v in var.frontend_ips : k => {
    name                          = k
    public_ip_address_id          = try(v.create_public_ip, false) ? azurerm_public_ip.new[k].id : try(data.azurerm_public_ip.exists[k].id, null)
  ...
  output_ips = { for _, v in azurerm_lb.lb.frontend_ip_configuration : v.name => coalesce(try(data.azurerm_public_ip.exists[v.name].ip_address, azurerm_public_ip.new[v.name].ip_address, v.private_ip_address)) }
...
resource "azurerm_network_security_rule" "allow_inbound_ips" {
  ...
  resource_group_name         = coalesce(var.network_security_resource_group_name, var.resource_group.name)
